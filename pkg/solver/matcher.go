...
func doOffersMatch(
	...
	if enableAllowlist {
		if !allowlist.IsModuleAllowed(jobOffer.Module) {
			log.Debug().
				Str("module", jobOffer.Module.Name).
				Msg("module not allowed by allowlist")
			return false
		}
	}
...
func getMatchingDeals(
	...
	for _, jobOffer := range jobOffers {
		...
		for _, resourceOffer := range resourceOffers {
			...
			if doOffersMatch(resourceOffer.ResourceOffer, jobOffer.JobOffer, enableAllowlist) {
